...
vddat1 <- temp_slopes %>% select(slope, slope_est, slope_se, slope_trim,
...
vddat2 <- vddat1 %>% filter(age == "Immature")

vddat1do <- do_slopes %>% select(slope, slope_est, slope_se, slope_trim,
  age, depth_iqr_scaled, depth_mean_scaled,
  log_age_scaled, growth_rate_scaled, max_mass_scaled,
  Zone, Latitude, Trophic, 
  chopstick, species, species_age) %>% 
  Hmisc::na.delete() 
vddat2do <- vddat1do %>% filter(age == "Immature")
...
# MEAN DEPTH ####
     tempslopemodv <- lmerTest::lmer(slope_est ~ 1 + 
         ...
         depth_mean_scaled * chopstick +
         (1|species), na.action = na.fail, REML = T, data = vddat2)
     
     tempslopemodv %>% summary()
     
(vp_depth_mean <- interactions::interact_plot(tempslopemodv, pred = depth_mean_scaled, 
  modx = chopstick, 
  ...
  vary.lty =TRUE) + 
    ...
    geom_point(data = filter(vddat2, chopstick == "low"),
      aes(depth_mean_scaled, slope_est
      ), #alpha = 0.2, 
...
(vp_depth_iqr <- interactions::interact_plot(tempslopemodvr, 
...
(vp_log_age <- interactions::interact_plot(
  tempslopemodva, # vel
  pred = log_age_scaled,
  modx = chopstick,
  interval = TRUE, int.type = c("prediction"), line.thickness = 0.5,
...
(vp_growth_rate <- interactions::interact_plot(tempslopemodvg, 
...
# MEAN DEPTH ####
     ...
     doslopemodv <- lmerTest::lmer(slope_est ~ 1 +
         depth_mean_scaled * chopstick +
         # (1|species) +
         (1|species_age), REML = T, na.action = na.fail, data = vddat2do)
     doslopemodv %>% summary()
     
(vpd_depth_mean <- interactions::interact_plot(doslopemodv, pred = depth_mean_scaled,
  modx = chopstick,
  ...
  vary.lty =TRUE
    ...
    geom_point(data = filter(     vddat2do, chopstick == "high"),
      aes(depth_mean_scaled, slope_est), 
      alpha = 1, size = 1.5, colour = "gold", shape = 21, 
      inherit.aes = F) +
    geom_linerange(data = filter(     vddat2do, chopstick == "high"),
      aes(x = depth_mean_scaled,
        ymin = (slope_est - slope_se * 1.96),
        ymax = (slope_est + slope_se * 1.96)),
      alpha = 1, fatten = 1, colour = "gold", shape = 21, inherit.aes = F) +
    geom_point(data = filter(     vddat2do, chopstick == "low"),
      aes(depth_mean_scaled, slope_est), 
      alpha = 1, size = 1.5, colour = "darkcyan", shape = 21, 
      inherit.aes = F) +
    geom_linerange(data = filter(     vddat2do, chopstick == "low"),
      aes(x = depth_mean_scaled,
        ymin = (slope_est - slope_se * 1.96),
        ymax = (slope_est + slope_se * 1.96)), 
...
(vpd_depth_iqr <- interactions::interact_plot(doslopemodvr, 
  pred = depth_iqr_scaled,
  modx = chopstick,
  # modx = growth_rate_scaled,
  interval = TRUE, int.type = c("prediction"), line.thickness = 0.5,
...
(vpd_log_age <- interactions::interact_plot(
  # doslopemod2b, 
  doslopemodva, 
  pred = log_age_scaled,
  modx = chopstick,
...
###
#### age by depth ####
(vpd_log_age2 <- interactions::interact_plot(doslopemod2b, 
  pred = log_age_scaled, 
  modx = depth_iqr_scaled,
  interval = TRUE, int.type = c("prediction"), line.thickness = 0.5, 
  outcome.scale = "response", plot.points = TRUE,
...
(vpd_growth_rate <- interactions::interact_plot(
  # doslopemod2,
  # pred = growth_rate_scaled, 

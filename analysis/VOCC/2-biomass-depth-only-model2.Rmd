...
data <- gfvelocities::scale_predictors(data, # predictors = c(quo(depth)))
...
Check raw data
```{r}
data %>% filter(year >2007) %>% ggplot() + geom_point(aes(X,Y, colour = density, size = density*1000), shape = 20, alpha=0.4) + 
  # geom_polygon(data = coast, aes_string(x = "X", y = "Y", group = "PID"),
  #   fill = "grey87", col = "grey70", lwd = 0.2) +
  scale_colour_viridis_c(trans = fourth_root_power, direction = -1) +
  coord_fixed(expand = F, 
    xlim = c(range(data$X)+ c(-0.001, 0.001)), ylim = c(range(data$Y)+ c(-0.001, 0.001))) + 
...
Make mesh
```{r}
# m_old <- readRDS(here::here(paste0("analysis/VOCC/data/", spp, "/mod-imm-biomass-", spp, "-tv-depth-only-1n3n4n16.rds")))
# data <- m_old$data
...
if (params$update_model) {
  ...
  if (any(names(data) == "adult_density")) {
   ...
   try({ imm_formula <- as.formula(paste(
      "imm_density ~ 0 + as.factor(year)", covariates, "+ depth_scaled + depth_scaled2"
    ))
    ...
    saveRDS(imm_biomass,
      file = paste0("data/", spp, "/mod-imm-biomass-",
        spp, covs, "-", ssid_string, "-2021-noRW.rds"))
   })
...
if (!exists("nd_all")) {
  ...
  dat <- gfvelocities::scale_predictors(dat, predictors = c(quo(depth)))
